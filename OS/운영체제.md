...
# Ch 11. Mass-Storage Structure (대용량 저장장치 구조)

> 큰 흐름: **Mass-Storage (대용량 저장장치)** 는 파일시스템/가상메모리/프로그램 실행의 “바닥”을 받치는 저장 계층이다. 핵심은 **Disk/SSD 구조 이해 → 어떻게 붙는지(Attachment) → 어떻게 읽고 쓰는 요청을 줄 세울지(Scheduling) → 어떻게 관리할지(Management/Swap) → 어떻게 신뢰성과 성능을 올릴지(RAID/Stable Storage)** 로 이어진다.

---

## 1. Overview (개요)

### 1.1 저장 계층(Storage Hierarchy) 관점

* 저장장치는 보통 **속도-용량-가격(비트당 비용)** 이 트레이드오프.
* 일반적 계층:

  * **Registers / Cache / Main Memory**
  * **Secondary Storage (2차 저장장치)**: HDD/SSD (대부분 “대용량”의 중심)
  * **Tertiary Storage (3차 저장장치)**: 테이프/광학 등(백업/아카이브)
* OS 관점에서 중요한 목표:

  * **Performance (성능)**: 평균 접근시간/처리량
  * **Reliability (신뢰성)**: 장애 허용, 데이터 보존
  * **Manageability (관리성)**: 초기화/부팅/스왑/확장

### 1.2 성능의 기본 분해

* 대용량 저장장치 I/O 시간은 보통 아래 요소들의 합으로 생각한다.

  * **Seek Time (탐색 시간)**: 원하는 트랙/실린더로 헤드를 옮기는 시간 (HDD에서 특히 큼)
  * **Rotational Latency (회전 지연)**: 원하는 섹터가 헤드 아래로 올 때까지 기다리는 시간 (HDD)
  * **Transfer Time (전송 시간)**: 실제 데이터를 읽고/쓰는 시간
  * (추가) **Controller/Queue Delay (컨트롤러/대기 지연)**: 요청이 밀릴 때 대기
* 따라서 “요청을 어떤 순서로 처리하느냐(스케줄링)”가 HDD 성능에 큰 영향을 준다.

---

## 2. Disk Structure (디스크 구조)

### 2.1 HDD의 물리 구조

* **Platter (플래터)**: 원판 여러 장
* **Spindle (스핀들)**: 회전축
* **Read/Write Head (헤드)**: 플래터 표면을 읽고 씀
* 데이터 배치 단위:

  * **Track (트랙)**: 원형 고리
  * **Sector (섹터)**: 트랙을 쪼갠 조각(최소 전송 단위의 핵심)
  * **Cylinder (실린더)**: 여러 플래터의 같은 반지름 트랙 묶음
* 중요한 포인트:

  * HDD는 “기계적 이동(Seek/Rotation)”이 크고 느리다 → 스케줄링이 중요.

### 2.2 논리 주소화(Logical Addressing)

* 옛 방식: **CHS (Cylinder-Head-Sector)** 로 직접 지정
* 현대: **LBA (Logical Block Addressing)**

  * 디스크를 “0..N-1 블록”처럼 보이게 하고, 내부 매핑은 컨트롤러/펌웨어가 처리.
* OS는 보통 “블록 디바이스”로 보고 블록 단위 I/O를 요청한다.

### 2.3 SSD(및 플래시)의 특징(개념적 비교)

* SSD는 기계적 Seek/Rotation이 거의 없다 → HDD식 스케줄링의 효과가 줄어듦.
* 대신 내부적으로는:

  * **Flash Translation Layer (FTL)** 같은 매핑/가비지 컬렉션/웨어 레벨링 이슈가 성능을 좌우.
* 그래도 OS 레벨에서 **queue depth**, **병렬성 활용**, **쓰기 패턴**은 여전히 중요할 수 있다.

---

## 3. Disk Attachment (디스크 연결 방식)

### 3.1 Host-Attached Storage (호스트 직결)

* 디스크가 특정 컴퓨터에 직접 연결(예: SATA/SAS, NVMe 등)
* 장점: 단순하고 빠른 경우가 많음
* 단점: 공유/확장에 제약

### 3.2 Network-Attached Storage (NAS) / Storage Area Network (SAN)

* **NAS**: “파일 단위” 공유(클라이언트는 파일로 접근)
* **SAN**: 네트워크를 통해 “블록 단위”로 디스크를 제공(호스트는 로컬 디스크처럼 사용)
* OS/시스템 관점 포인트:

  * 공유/확장/관리 편의성 vs 네트워크 지연/복잡도

---

## 4. Disk Scheduling (디스크 스케줄링)

> 목표: **헤드 이동(Seek)과 회전 대기**를 줄여 평균 응답시간/처리량을 개선. (특히 HDD)

### 4.1 스케줄링이 필요한 이유

* 여러 프로세스가 동시에 I/O 요청 → 디스크 큐에 요청이 쌓임
* “큐 순서”에 따라 헤드 이동량이 크게 달라짐
...
* 도착 순서대로 처리
* 장점: 공정, 구현 쉬움
* 단점: 헤드 이동이 커질 수 있어 평균 성능이 나쁠 수 있음

#### (2) SSTF (Shortest Seek Time First) 최단 탐색 우선

* 현재 헤드 위치에서 **가장 가까운 요청**부터 처리
* 장점: 평균 Seek 감소
* 단점: **Starvation (기아)** 가능(멀리 있는 요청이 계속 밀릴 수 있음)

#### (3) SCAN (엘리베이터) / LOOK

* 헤드가 한 방향으로 쭉 이동하며 요청 처리, 끝에 닿으면 방향 전환
* **SCAN**: 디스크 끝까지 갔다가 반대로
* **LOOK**: 실제 요청이 있는 마지막 지점까지만 갔다가 반대로 (불필요 이동 감소)
* 장점: SSTF보다 기아 완화, 처리 패턴 예측 가능

#### (4) C-SCAN / C-LOOK (Circular)

* 한 방향으로만 처리하고, 끝에 도달하면 **처리 없이 반대 끝으로 점프**(원형처럼)
* 장점: 대기시간이 위치에 따라 덜 치우쳐서 **응답시간의 균등성**이 좋아짐
* **C-LOOK**: 요청 있는 범위까지만 갔다가 점프

### 4.3 스케줄링의 실전 포인트

* “최적”은 미래 요청을 알아야 하므로 현실적으로 불가.
* HDD에서 특히 효과 큼. SSD/NVMe는 다른 병목(내부 병렬성/컨트롤러/쓰기 증폭 등)이 더 중요할 수 있음.
* OS는 디바이스/드라이버 계층에서 스케줄링을 하거나, 장치 자체 컨트롤러가 큐를 재정렬하기도 한다.

---

## 5. Disk Management (디스크 관리)

### 5.1 Formatting (포맷) 단계

* **Low-Level Formatting (저수준 포맷)**

  * 섹터/헤더 등 물리적 구조를 준비(현대 디스크는 제조 단계에서 대부분 수행)
* **Partitioning (파티셔닝)**

  * 디스크를 여러 구획으로 나눔(각 파티션을 독립 디스크처럼 사용)
* **Logical Formatting (논리 포맷)**

  * 파일시스템 구조(슈퍼블록, FAT/inode 등)를 생성

### 5.2 Boot Block / Bootstrapping (부팅 관련)

* 시스템 부팅을 위해 디스크의 특정 위치에 **Boot Block(부트 코드)** 이 존재
* 부트스트랩 흐름(개념):

  * 펌웨어/부트로더가 커널을 로드 → 커널이 장치/파일시스템 초기화

### 5.3 Bad Blocks (불량 블록) 처리

* 디스크는 시간이 지나며 불량 섹터가 생길 수 있음
* 일반적으로 컨트롤러/펌웨어 수준에서 **Sector Sparing/Remapping (대체 섹터로 치환)** 등을 수행
* OS는 “논리 블록” 관점에서 최대한 연속성을 기대하지만, 내부 치환으로 성능이 흔들릴 수 있음

---

## 6. Swap-Space Management (스왑 공간 관리)

### 6.1 Swap Space (스왑 공간) 역할

* 메모리가 부족할 때 프로세스/페이지를 디스크로 내보내는 공간
* 가상메모리 구현에서:

  * 페이지 단위 스왑(요구 페이징)과 연결됨
* 목표:

  * 스왑 I/O는 빈번하고 성능 민감 → 가능한 빠른 디스크 영역/전용 파티션을 쓰기도 함

### 6.2 구현 방식(개념)

* **Swap Partition (전용 파티션)**: 파일시스템을 거치지 않아 오버헤드가 적을 수 있음
* **Swap File (스왑 파일)**: 관리/확장이 편함(대신 파일시스템 경로/단편화 영향 가능)
* 무엇을 선택하든 OS는 스왑을 “특수한 대량 블록 I/O”로 보고 최적화하려 한다.

---

## 7. RAID Structure (RAID 구조)

> 큰 흐름: 여러 디스크를 묶어 **성능(병렬성)** 과 **신뢰성(중복)** 을 얻는 기법.

### 7.1 왜 RAID인가?

* 디스크는 대용량이지만 개별 장치 장애 확률이 존재
* 단일 디스크는 I/O 병렬성이 제한됨
* RAID는:

  * **Redundancy (중복)** 로 장애 허용
  * **Parallelism (병렬)** 로 처리량 개선(특히 읽기)

### 7.2 핵심 기법

* **Mirroring (미러링)**: 같은 데이터를 두 곳에 저장
* **Striping (스트라이핑)**: 데이터를 여러 디스크에 나눠 병렬 접근
* **Parity (패리티)**: 오류 복구용 정보를 추가로 저장

### 7.3 자주 언급되는 RAID 레벨(개념 요약)

* **RAID 0**: 스트라이핑만(성능↑, 중복 없음 → 신뢰성↓)
* **RAID 1**: 미러링(신뢰성↑, 저장 효율은 50% 수준)
* **RAID 4/5**: 패리티 기반(특히 RAID 5는 분산 패리티로 병목 완화)
* **RAID 6**: 이중 패리티(동시 2디스크 장애까지 견딤)
* 포인트:

  * 패리티 RAID는 **쓰기 성능/복구 비용** 이 설계상 고려사항이 된다.

---

## 8. Stable Storage (안정 저장장치) 개념

### 8.1 Stable Storage가 다루는 문제

* 전원 장애/부분 쓰기/컨트롤러 문제 등으로 “쓰기 도중 깨짐”이 발생할 수 있음
* 목표: 실패가 있어도 **일관된 상태**를 유지(또는 복구 가능)하게 만들기

### 8.2 전형적 아이디어(개념)

* **Replication (복제)**: 같은 데이터를 여러 곳에 기록
* **Write Protocol (쓰기 프로토콜)**:

  * 순서를 정해 “둘 다 성공하면 완료, 아니면 복구 루틴” 같은 방식으로 원자성에 가까운 동작을 구현
* 이 개념은 실제로는 파일시스템/DB의 저널링, 로그 구조와도 연결된다(“안정적으로 쓰는 방법”이라는 공통 관심사).

---

## 9. Summary (요약)

* 대용량 저장장치는 저장 계층의 핵심이며, 성능은 **탐색/회전/전송/대기**로 분해해 이해한다.
* HDD는 기계적 지연이 커서 **Disk Scheduling(FCFS, SSTF, SCAN/C-SCAN, LOOK/C-LOOK)** 이 큰 차이를 만든다.
* 디스크 관리는 **포맷(저수준/파티션/논리)**, **부팅(boot block)**, **불량 블록 처리** 같은 운영 이슈를 포함한다.
* **Swap space** 는 가상메모리와 맞물려 성능 민감한 특수 저장 영역이며 구현(전용 파티션/파일) 선택이 중요하다.
* **RAID** 는 여러 디스크를 묶어 성능과 신뢰성을 끌어올리고, **Stable Storage** 는 장애 상황에서도 일관성을 유지하도록 “안전한 쓰기”를 설계한다.

# Ch 12. I/O Systems (입출력 시스템)

> 큰 흐름: **I/O System(입출력 시스템)** 은 CPU/메모리/프로세스와 **Device(장치)** 사이에서 데이터를 안전하고 효율적으로 이동시키는 OS의 핵심 인프라다. 큰 축은 **(1) I/O 하드웨어 구조 이해 → (2) Interrupt/DMA로 데이터 이동 → (3) Device Driver로 장치 추상화 → (4) 커널 I/O 서브시스템(버퍼링/캐싱/스풀링/에러처리/보호) → (5) 성능(지연/처리량)과 튜닝** 이다.

---

## 1. Overview (개요)

### 1.1 OS에서 I/O가 중요한 이유

* 실제 시스템 성능은 CPU만이 아니라 **I/O 지연**에 크게 좌우된다.
* 장치는 속도/특성이 매우 다양하고, 오류도 잦다 → OS가 **표준화된 인터페이스**와 **복구/보호**를 제공해야 한다.

### 1.2 I/O 시스템의 목표

* **Abstraction (추상화)**: 장치별 차이를 숨기고 공통 인터페이스 제공
* **Efficiency (효율)**: CPU 개입 최소화(DMA 등), 병렬성 활용
* **Reliability (신뢰성)**: 오류 검출/재시도/복구
* **Protection & Security (보호/보안)**: 권한 없는 I/O 방지

---

## 2. I/O Hardware (입출력 하드웨어)

### 2.1 기본 구성 요소

* **Device (장치)**: 디스크, 키보드, NIC 등
* **Device Controller (장치 컨트롤러)**: 장치를 제어하는 전자 회로/펌웨어
* **Bus (버스)**: CPU/메모리/장치 연결 통로(예: PCIe 등)

### 2.2 Port-mapped I/O vs Memory-mapped I/O

* **Port-mapped I/O**: 별도 I/O 주소 공간에 접근(특수 명령으로 읽기/쓰기)
* **Memory-mapped I/O (MMIO)**: 장치 레지스터를 **메모리 주소 공간**에 매핑해 일반 load/store로 접근

  * 장점: 프로그래밍 모델 단순
  * 주의: 캐시/재정렬 문제 → 메모리 배리어 등 고려(아키텍처/OS 정책)

### 2.3 장치 레지스터와 명령

* 컨트롤러는 보통 다음을 노출:

  * **Status register (상태 레지스터)**: 준비됨/에러 등
  * **Control/Command register (제어/명령)**: 읽기/쓰기 시작
  * **Data register/buffer (데이터)**: 실제 데이터 이동

---

## 3. Interrupts (인터럽트)

### 3.1 Interrupt 기반 I/O의 핵심 아이디어

* 장치가 이벤트/완료를 CPU에 알림 → CPU는 그때만 처리
* **Polling(폴링)** 대비 CPU 낭비를 줄인다.

### 3.2 인터럽트 처리 흐름(전형)

1. 장치가 I/O 완료/이벤트 발생
2. 컨트롤러가 **Interrupt Request (IRQ)** 발생
3. CPU가 현재 실행을 잠시 멈추고 커널의 **Interrupt Handler (ISR)** 로 점프
4. ISR이 상태 확인/데이터 일부 처리/작업 완료 표시
5. 필요하면 대기 중인 프로세스 깨움(wakeup)
6. 원래 실행으로 복귀

### 3.3 Interrupt Vector & Interrupt Priority

* **Interrupt Vector (인터럽트 벡터)**: 장치별로 적절한 핸들러 주소로 분기
* **Priority (우선순위)**: 중요한/긴급한 인터럽트가 먼저 처리되도록 설계 가능
...
* 고속 장치(디스크/NIC)에서 매 바이트를 CPU가 복사하면 CPU가 병목이 됨
* **DMA**는 컨트롤러가 메모리로 직접 전송해 CPU 개입을 최소화

### 4.2 DMA 동작 개요

1. CPU/드라이버가 DMA 컨트롤러(또는 장치)에 **전송 주소/크기/방향** 설정
2. 장치가 메모리로 직접 읽기/쓰기 수행
3. 완료 시 인터럽트로 CPU에 알림

### 4.3 DMA에서 자주 함께 나오는 이슈(개념)

* **Buffer pinning (고정)**: DMA 중인 메모리 페이지가 스왑아웃되지 않게 고정
* **Cache coherency (캐시 일관성)**: CPU 캐시와 DMA가 보는 메모리가 다를 수 있음 → flush/invalidate 필요할 수 있음

---

## 5. I/O Software (입출력 소프트웨어) 계층

### 5.1 OS가 제공해야 할 공통 기능

* 장치별 차이를 숨김(표준 API)
* 동기화/경쟁 제어(여러 프로세스가 같은 장치를 쓰려는 상황)
* 오류 처리/재시도/로깅

### 5.2 I/O 명령 방식

* **Synchronous I/O (동기 I/O)**: 요청한 스레드가 완료까지 기다림
* **Asynchronous I/O (비동기 I/O)**: 요청만 걸고, 완료는 이벤트/콜백/완료 큐로 수령
* **Blocking vs Nonblocking**

  * Blocking: 데이터 없으면 잠듦
  * Nonblocking: 즉시 반환(EAGAIN 등) 후 나중에 재시도

---

## 6. Device Drivers (장치 드라이버)
...
* OS 커널과 장치 컨트롤러 사이에서:
  ...
  * 인터럽트 처리 연결
  * 오류 복구
* 드라이버 덕분에 커널 상위 계층은 “장치 구체 구현”을 몰라도 된다.

### 6.2 Character vs Block Devices (개념)

* **Character device**: 바이트 스트림 중심(키보드, 시리얼 포트 등)
* **Block device**: 블록 단위 접근(디스크)

  * 캐싱/버퍼링/스케줄링 등과 강하게 결합

---

## 7. Kernel I/O Subsystem (커널 I/O 서브시스템)

> 드라이버 위에서 OS가 제공하는 “공통 고급 기능” 레이어.

### 7.1 I/O Scheduling (I/O 스케줄링)

* 디스크 같은 블록 장치에서 요청을 재정렬해 지연/처리량 최적화
* (11장) HDD의 SCAN/LOOK 같은 개념과 연결되며, SSD는 다른 병목이 더 중요할 수 있음

### 7.2 Buffering (버퍼링)

* **Buffer (버퍼)**: 전송 중간에 임시 저장 공간을 둬서 속도/단위 차이를 흡수
* 왜 필요?

  * **속도 불일치**: CPU는 빠르고 장치는 느림
  * **전송 단위 불일치**: 네트워크 패킷 vs 사용자 요청 크기
  * **복사/정렬 필요**: 커널/유저 공간 경계에서 데이터 이동

### 7.3 Caching (캐싱)

* **Cache (캐시)**: 자주 쓰는 데이터를 메모리에 보관해 반복 I/O를 줄임
* 디스크 블록 캐시/페이지 캐시가 대표적
* 주의: **Consistency (일관성)**

  * 쓰기 지연(write-back) vs 즉시 반영(write-through)

### 7.4 Spooling (스풀링)

* 한 번에 한 프로세스만 사용할 수 있는 장치(예: 프린터)를 위해

  * 출력 데이터를 디스크에 모아두고
  * 순차적으로 장치에 흘려보내는 방식
* 목표: **공유/병렬성/사용자 경험** 개선

### 7.5 Device Reservation & Sharing (장치 예약/공유)

* 어떤 장치는 **독점 사용**이 필요(테이프 드라이브 등)
* 어떤 장치는 **공유 가능**(디스크는 블록 단위로 다중 접근)
* OS는 정책/락/권한으로 조정

### 7.6 Error Handling (오류 처리)

* I/O는 오류가 흔함: 시간초과, CRC 오류, 장치 분리 등
* OS/드라이버는 보통:
...
### 7.7 Protection & Access Control (보호)

* 유저 프로세스가 임의로 장치 레지스터를 만지면 시스템이 망가질 수 있음
* 그래서:

  * **Privileged instructions (특권 명령)**
  * **I/O permission bitmap / capability**
  * **Device file 권한** 등으로 제어

### 7.8 Power Management (전원 관리)

* 장치 전원 상태 제어(절전/유휴/활성)
* 모바일/서버 모두에서 중요한 운영 요소

---

## 8. I/O Performance (입출력 성능)

### 8.1 측정 지표

* **Latency (지연 시간)**: 요청→응답까지 시간
* **Throughput (처리량)**: 단위 시간당 처리되는 데이터/요청 수
* **CPU overhead (CPU 오버헤드)**: I/O 때문에 CPU가 쓰는 시간
...
* **Batching/Coalescing (묶기)**: 작은 I/O를 합쳐 오버헤드 감소
* **Asynchronous I/O**: 대기 시간을 다른 작업과 겹치기
* **DMA 활용**: CPU 복사 최소화
* **캐싱/프리페치**: 반복 접근 줄이기
...
* I/O 시스템은 다양한 장치를 표준화된 방식으로 다루기 위해 **하드웨어(컨트롤러/버스/MMIO) + 인터럽트/DMA + 드라이버 + 커널 I/O 서브시스템**으로 구성된다.
* **Interrupt** 는 이벤트 기반으로 CPU 낭비를 줄이고, **DMA** 는 대량 전송에서 CPU 병목을 줄인다.
* 커널 I/O 서브시스템은 **스케줄링, 버퍼링, 캐싱, 스풀링, 보호, 에러처리** 같은 공통 기능을 제공한다.
* 성능은 **지연/처리량/CPU 오버헤드**로 보고, 비동기화·배칭·DMA·캐싱 같은 기법으로 최적화한다.

---

# Ch 13. File-System Interface (파일 시스템 인터페이스)

> 큰 흐름: **File System(파일 시스템)** 은 저장장치 위에 **파일/디렉터리라는 추상화**를 제공해 프로그램이 데이터를 쉽게 저장·검색·공유하게 한다. 13장은 구현(어떻게 배치/할당/저널링)보다, **사용자·프로그램이 보게 되는 인터페이스**에 집중한다. 핵심은 **(1) 파일의 개념과 연산 → (2) 접근 방법 → (3) 디렉터리/경로 구조 → (4) 마운팅과 파일시스템 구성 → (5) 공유와 보호(권한)** 이다.
...
* 관련된 정보의 **논리적 저장 단위**
* OS 관점에서 파일은 보통 다음으로 모델링된다.

  * **Name (이름)**: 사람이 읽는 식별자
  * **Identifier (식별자)**: 내부적으로 파일을 가리키는 고유 ID(예: inode 번호)
  * **Type (종류)**: 실행파일/텍스트/디렉터리 등
  * **Location (위치)**: 저장장치 상의 블록/포인터 정보
  * **Size (크기)**
  * **Protection (보호 정보)**: 누가 어떤 권한으로 접근 가능한지
  * **Time/Date/User IDs (타임스탬프/소유자)** 등 메타데이터

### 1.2 File Operations (파일 연산)

* OS가 제공하는 기본 연산(시스템콜/라이브러리 형태)

  * **create, open, read, write, close**
  * **delete, truncate**(내용 제거), **seek/reposition**(파일 오프셋 이동)
  * **get/set attributes**(메타데이터 조회/설정)

### 1.3 Open Files (열린 파일)과 Open-File Table

* `open()` 후 `read()/write()` 를 반복할 때마다 경로 탐색을 다시 하지 않기 위해 OS는 **Open-file table** 을 유지
* 전형적으로 관리되는 정보
  ...
  * 파일 참조 카운트
  * 메모리 내 캐시/락 관련 상태
* 보통 **시스템 전역 테이블 + 프로세스별 테이블**(FD: file descriptor) 구조를 갖는다.
...
* 파일을 앞에서부터 차례대로 읽고/쓴다.
* 연산 모델: `read next`, `write next`, `rewind` 등
* 로그/텍스트 처리처럼 “흐름” 기반 작업에 적합

### 2.2 Direct (Random) Access (직접 접근)

* 파일을 **레코드/바이트 오프셋** 기준으로 임의 위치에서 접근
* 연산 모델: `read n`, `write n` (n번째 블록/레코드)
* 데이터베이스/대형 바이너리처럼 랜덤 액세스가 중요한 경우에 적합

### 2.3 Indexed Access (인덱스 접근)

* 인덱스를 통해 특정 위치로 빠르게 점프하는 방식
* 개념적으로는 “색인(테이블) → 실제 데이터 위치” 매핑
* 실제 구현은 이후 장(파일시스템 구현)에서 인덱스 블록, 다단계 인덱스 등으로 등장

---

## 3. Directory and Disk Structure (디렉터리와 경로 구조)

### 3.1 Directory (디렉터리)의 역할

* 파일 이름을 파일의 내부 식별자(또는 메타데이터)로 매핑하는 **네임스페이스**
* 제공해야 할 기본 연산

  * **search**(찾기), **create**, **delete**, **list**
  * **rename**, **traverse**(경로 따라가기)

### 3.2 Directory Structures (디렉터리 구조 유형)

#### (1) Single-Level Directory (단일 레벨)

* 모든 파일이 한 디렉터리에 존재
* 단순하지만 이름 충돌/확장성 문제 큼

#### (2) Two-Level Directory (2단계)

* 사용자별로 독립 디렉터리(MFD/UFD 개념)
* 사용자 간 이름 충돌 완화, 공유가 번거로울 수 있음

#### (3) Tree-Structured Directory (트리 구조)

* 오늘날 표준적인 구조
* **Absolute path(절대 경로)** / **Relative path(상대 경로)**
* 작업 디렉터리(현재 디렉터리) 개념과 잘 맞음

#### (4) Acyclic-Graph Directory (비순환 그래프)

* 파일/디렉터리를 **공유**하기 위해 한 노드가 여러 부모를 가질 수 있음(링크)
* 장점: 공유 용이
* 과제: 삭제/참조 관리(누가 마지막 참조자인지)

#### (5) General Graph Directory (일반 그래프)

* 순환(cycle)까지 허용
* 순회/삭제가 복잡해짐 → 순환 감지, 가비지 컬렉션 같은 추가 메커니즘 필요

### 3.3 Links (링크)

* **Hard link**: 같은 파일(같은 식별자/메타데이터)을 다른 이름으로 추가

  * 참조 카운트로 삭제 시점을 결정하는 방식이 흔함
* **Symbolic link (soft link)**: “경로 문자열”을 가리키는 특수 파일

  * 끊어진 링크(dangling) 가능
  * 파일시스템 경계를 넘는 링크에 유리한 경우가 많음

---

## 4. File-System Mounting (마운팅)

### 4.1 Mount (마운트) 개념

* 여러 저장장치/파티션/네트워크 파일시스템을 하나의 디렉터리 트리에 **붙여서** 사용
* **mount point**: 다른 파일시스템이 트리에 연결되는 지점

### 4.2 Mount 과정에서 OS가 확인하는 것(개념)

* 해당 장치/파티션에 **유효한 파일시스템** 이 존재하는지(슈퍼블록 등)
* 권한/옵션(읽기 전용 등)
* 마운트 테이블 갱신(“어떤 경로가 어떤 파일시스템으로 연결되는지”)
...
* 같은 머신에서 여러 사용자/프로세스가 같은 파일을 사용
* 네트워크 파일시스템(NFS/SMB 등)으로 원격 공유

### 5.2 Concurrent Access (동시 접근)과 일관성

* 여러 프로세스가 동시에 쓰면 **Race condition** 발생 가능
* OS/응용은 보통 다음을 조합해 해결

  * **File locking (파일 락)**: advisory/mandatory 등 정책에 따라 다름
  * 원자적 연산(append, rename 등 파일시스템이 보장하는 atomicity)
  * 애플리케이션 레벨 동기화

### 5.3 Remote File Systems (원격 파일 시스템)에서의 고려

* 네트워크 지연/분할, 캐시로 인해 **일관성 모델**이 중요해짐
* “즉시 반영” vs “캐시 후 지연 반영” 중 어떤 semantics를 제공하는지에 따라 프로그래밍 모델이 달라짐
...
* 파일은 공유 자원 → 무단 읽기/변조/삭제를 막아야 함

### 6.2 Access Types (접근 권한 종류)

* 전형적으로 다음 권한을 구분

  * **read, write, execute**
  * **append**, **delete**, **list** 등 세분화 가능

### 6.3 Access Control Mechanisms (권한 메커니즘)

#### (1) ACL (Access Control List)

* 파일마다 “사용자/그룹 → 권한” 목록을 둠
* 장점: 표현력이 높음
* 단점: 관리/검사 비용이 커질 수 있음

#### (2) Owner/Group/Other + Permission Bits (UNIX 스타일)

* 소유자/그룹/기타에 대한 권한 비트로 표현
* 장점: 간단하고 빠름
* 단점: ACL보다 표현력이 제한적

#### (3) Capability (역량/토큰) 기반

* “객체가 누가 접근 가능한지”가 아니라 “주체가 무엇에 접근할 수 있는지”를 토큰처럼 들고 다님
* 위임/분산 환경에서 장점이 있지만 관리 방식이 달라짐

---

## 7. Summary (요약)

* 13장은 파일시스템의 **사용자 관점 인터페이스**: 파일/디렉터리/경로/권한/공유를 정리한다.
* 파일은 메타데이터와 함께 관리되며, 기본 연산은 **open–read/write–close** 흐름과 **open-file table** 로 최적화된다.
* 접근 방법은 **순차/직접/인덱스**로 나뉘고, 디렉터리는 **트리 구조**를 중심으로 링크/공유를 확장한다.
* 마운팅은 여러 파일시스템을 하나의 트리에 통합하는 핵심 메커니즘이다.
* 공유 환경에서는 **동시성(락/원자성)** 과 **보호(ACL/권한 비트/Capability)** 가 설계의 중심이 된다.

---

# Ch 14. File-System Implementation (파일 시스템 구현)

> 큰 흐름: 13장이 “**파일/디렉터리를 어떻게 보이게 할까(인터페이스)**”였다면, 14장은 “그걸 **디스크에 어떻게 저장하고 빠르게 관리할까(구현)**”다. 핵심은 **(1) 온디스크/인메모리 구조 → (2) 디렉터리 구현 → (3) 파일 할당 방법(연속/연결/인덱스) → (4) 빈 공간 관리 → (5) 성능(캐시/프리페치/클러스터링)과 신뢰성(일관성 검사/저널링 개념)** 이다.

---

## 1. File-System Structure (파일 시스템 구조)

### 1.1 Layered File System (계층 구조)

* 파일시스템은 보통 계층적으로 구성된다.

  * **I/O Control**: 장치 드라이버, 인터럽트/DMA
  * **Basic File System**: 블록 읽기/쓰기, 버퍼 캐시
  * **File-Organization Module**: 블록 할당/자유 공간 관리, 파일 블록 매핑
  * **Logical File System**: 디렉터리, 권한/보호, FCB(inode) 관리
* 목적: 하드웨어 차이를 숨기고(추상화), 정책과 메커니즘을 분리

### 1.2 On-Disk Structures (디스크 상 구조)

파일시스템은 디스크에 다음과 같은 “메타데이터 + 데이터” 레이아웃을 둔다.

* **Boot Control Block (부트 제어 블록)**: 부팅에 필요한 정보(있을 수도/없을 수도)
* **Volume Control Block (볼륨 제어 블록)**: 파일시스템 전체 메타데이터

  * 예: 총 블록 수, 블록 크기, 자유 공간 정보 위치 등
  * UNIX 계열에서 흔히 **superblock** 개념
* **Directory Structure (디렉터리 구조)**: 이름 → 파일 식별자(FCB) 매핑
* **File Control Block (FCB)**: 파일의 메타데이터(크기/소유자/권한/시간/데이터 위치 포인터)

  * UNIX의 **inode**가 대표
* **Data Blocks (데이터 블록)**: 실제 파일 내용

### 1.3 In-Memory Structures (메모리 상 구조)

* 디스크 메타데이터를 효율적으로 다루기 위해 OS는 다음을 메모리에 유지한다.

  * **Mount table (마운트 테이블)**: 어떤 파일시스템이 어디에 붙었는지
  * **Directory cache / name cache**: 경로 탐색 비용 감소
  * **System-wide open-file table** + **Per-process FD table**
  * **Buffer cache / Page cache**: 디스크 블록/페이지 캐싱

---

## 2. Directory Implementation (디렉터리 구현)

### 2.1 디렉터리는 무엇을 저장하나?

* 디렉터리 엔트리의 본질: **(file name → file identifier)**

  * identifier는 inode 번호/FCB 포인터 등

### 2.2 대표 구현 방식

#### (1) Linear List (선형 리스트)

* 디렉터리를 (이름, 식별자) 엔트리의 단순 배열/리스트로 저장
* 장점: 구현 단순
* 단점: 탐색이 **O(n)** → 파일이 많을수록 느림

#### (2) Hash Table (해시 테이블)

* 이름을 해시해 빠르게 찾음
* 장점: 평균적으로 탐색이 빠름
* 단점: 충돌 처리, 재해싱, 디스크 상 저장 구조 설계가 복잡

> 실전에서는 디렉터리 캐시 + 더 효율적인 인덱싱 구조(트리 등)를 결합해 성능을 끌어올린다.

---

## 3. Allocation Methods (파일 할당 방법)

> 파일의 “논리 블록 i”가 디스크의 “어느 물리 블록”에 있는지 매핑하는 핵심 설계.

### 3.1 Contiguous Allocation (연속 할당)

* 파일을 디스크의 **연속된 블록 구간**에 저장
* 디렉터리/FCB에 (start block, length)만 저장하면 됨
* 장점
...
* 단점
  ...
  * **External Fragmentation (외부 단편화)**
  * 파일 크기 증가(확장)에 취약(미리 크기 예측 필요)

### 3.2 Linked Allocation (연결 할당)

* 파일 블록이 흩어져 있어도 되고, 각 블록이 다음 블록 포인터를 가짐
* 장점
...
* 단점
  ...
  * 랜덤 접근이 매우 비효율적(앞에서부터 따라가야 함)
  * 포인터 오버헤드/신뢰성(포인터 손상 시 복구 어려움)

#### FAT (File Allocation Table) 변형

* “다음 블록 포인터”를 블록 안이 아니라 디스크의 별도 테이블(FAT)에 모아둠
* 장점: 포인터 따라가기가 더 빠르고, 디스크 접근 패턴이 개선될 수 있음
* 단점: FAT가 커질수록 메모리/갱신 비용 증가

### 3.3 Indexed Allocation (인덱스 할당)

* 파일마다 **index block(인덱스 블록)** 을 두고, 그 안에 데이터 블록 포인터 배열 저장
* 장점
...
* 단점
  ...
  * 인덱스 블록 오버헤드(작은 파일에 비효율)

#### Multi-level Index / Inode 방식(개념)

* 포인터가 많아지면 인덱스를 여러 단계로(예: 1단/2단/3단)
* UNIX inode는 흔히

  * **Direct pointers** (바로 데이터 블록)
  * **Single/Double/Triple indirect** (간접 인덱싱)
  * 로 큰 파일까지 확장

---

## 4. Free-Space Management (자유 공간 관리)

> 새 파일 생성/파일 확장 시 “빈 블록을 빠르게 찾아 할당”하는 문제.

### 4.1 Bit Vector (비트맵)

* 각 블록에 대해 0/1로 사용 여부 표시
* 장점: 구현 단순, 연속 공간 탐색에도 유리
* 단점: 디스크가 매우 크면 비트맵도 커짐(하지만 캐시로 완화 가능)

### 4.2 Linked List (자유 리스트)

* 빈 블록들을 연결 리스트로 연결
* 장점: 단순
* 단점: 연속 공간 찾기 어렵고, 탐색 비용이 클 수 있음

### 4.3 Grouping / Counting (그룹핑/카운팅)

* **Grouping**: 한 블록에 “다음 빈 블록들의 목록”을 저장해 탐색 효율을 높임
* **Counting**: (start, count)로 “연속된 빈 블록 구간”을 기록

  * 연속 할당/클러스터링과 궁합이 좋음

---

## 5. Efficiency & Performance (효율과 성능)

### 5.1 캐시의 중심: Buffer Cache / Page Cache

* 디스크 접근은 느리므로, OS는 **최근/빈번 블록**을 메모리에 캐싱
* 읽기 성능을 극적으로 올리며, 쓰기도 지연(write-back)으로 묶어서 최적화 가능

### 5.2 Read-Ahead / Prefetch (선읽기)

* 순차 접근이 예상되면 다음 블록을 미리 읽어두어 지연을 숨김
* 지역성(Locality)을 적극 활용

### 5.3 Clustering (클러스터링)

* 논리적으로 연속된 블록을 물리적으로 가깝게 배치해

  * 탐색(seek) 비용을 줄이고
  * 대역폭을 끌어올림

### 5.4 Metadata Performance (메타데이터 성능)

* 실제 체감은 “데이터 읽기”뿐 아니라

  * 경로 탐색, inode/FCB 접근, 디렉터리 검색 등 메타데이터 작업이 좌우
* 그래서 **name cache / inode cache** 같은 구조가 중요

---

## 6. Recovery & Consistency (복구와 일관성)

### 6.1 Consistency Checking (일관성 검사)

* 크래시(전원 장애 등)로 메타데이터 갱신이 중간에 끊기면

  * 디렉터리-인덱스-자유공간 정보가 서로 불일치할 수 있음
* 전통적 해결: 재부팅 시 **fsck/검사 도구**로 메타데이터를 스캔해 복구

  * 단점: 디스크가 크면 매우 오래 걸릴 수 있음

### 6.2 Journaling (저널링) 개념(맛보기)

* 변경 사항을 “로그(저널)”에 먼저 기록하고 커밋한 뒤 실제 구조에 반영
* 크래시 후에는 저널을 기반으로 빠르게 복구 가능
* (세부 기법은 이후 장/파일시스템 종류에서 더 깊게 다룸)

---

## 7. Summary (요약)

* 14장은 파일시스템을 **디스크에 어떻게 배치하고 메모리에서 어떻게 관리하는지**(온디스크/인메모리 구조)를 다룬다.
* 디렉터리는 이름→식별자 매핑이며, 구현은 **선형 리스트**부터 **해시/인덱싱**까지 확장된다.
* 파일 할당은 **연속/연결(FAT)/인덱스(inode, 다단계)** 가 대표이며, 각각 단편화·랜덤접근·오버헤드 트레이드오프가 있다.
* 빈 공간 관리는 **비트맵/자유리스트/그룹핑/카운팅**으로 빠른 할당을 지원한다.
* 성능은 **캐싱/선읽기/클러스터링/메타데이터 캐시**가 좌우하며,
  신뢰성은 **일관성 검사**와 **저널링** 같은 복구 메커니즘으로 보강한다.

--- 

# Ch 15. File-System Internals (파일 시스템 내부)

> 큰 흐름: 13·14장이 “인터페이스/기본 구현(할당·자유공간)”을 다뤘다면, 15장은 **현대 파일시스템이 내부에서 실제로 굴러가는 핵심 메커니즘**을 묶어 정리한다. 중심 축은 **(1) 볼륨·파티션·마운트와 메타데이터 구조 → (2) VFS(가상 파일시스템)로 다양한 FS 추상화 → (3) 버퍼/페이지 캐시와 I/O 경로 → (4) 일관성·복구(저널링/로그) → (5) 고급 구조(Extent, Copy-on-Write, 스냅샷 등)** 이다.

---

## 1. Volumes and File Systems (볼륨과 파일시스템)

### 1.1 Disk Partitions (파티션)

* 물리 디스크는 보통 여러 **Partition(파티션)** 으로 나뉜다.
* 각 파티션은 독립적인 파일시스템을 가질 수 있고, OS는 이를 별도 저장장치처럼 다룬다.

### 1.2 Volume (볼륨)

* **Volume** 은 파일시스템이 올라가는 “논리 저장 단위”

  * 단일 파티션일 수도
  * 여러 디스크/파티션을 묶은 논리 단위일 수도 있음(볼륨 매니저/RAID 등과 결합)

### 1.3 Mounting Revisited (마운트 복습)

* 파일시스템을 디렉터리 트리에 붙이는 과정
* 커널은 마운트 시

  * 볼륨 제어 블록(예: superblock) 읽기
  * 루트 디렉터리 식별
  * 마운트 테이블 갱신
  * 옵션(읽기 전용 등) 적용
...
## 2. Virtual File System (VFS, 가상 파일시스템)

### 2.1 왜 VFS가 필요한가?

* 시스템에는 여러 종류의 파일시스템이 공존 가능

  * 로컬 디스크 파일시스템, 네트워크 파일시스템, 특수 파일시스템 등
* 사용자/프로그램은 `open/read/write/close` 같은 **일관된 API**를 기대

### 2.2 VFS의 핵심 아이디어

* “공통 인터페이스”를 정의하고, 실제 파일시스템별 구현을 플러그인처럼 붙인다.
* 전형적인 구조

  * **VFS interface**: 공통 연산(lookup, create, read, write, mkdir, unlink …)
  * **vnode/inode abstraction**: 파일 객체를 가리키는 커널 내부 표현
  * **filesystem-specific operations**: ext 계열, FAT 계열, 네트워크 FS 등 구체 동작

### 2.3 이름 해석(Name Resolution)과 VFS

* 경로 `/a/b/c` 를 처리할 때

  * 디렉터리 엔트리 탐색(lookup)
  * 마운트 포인트를 만나면 **다른 파일시스템으로 전환**
* 이 과정에서 **name cache(디렉터리 캐시)**, **inode/vnode cache**가 성능에 결정적

---

## 3. Caching and I/O Path (캐싱과 I/O 경로)

### 3.1 Buffer Cache vs Page Cache

* **Buffer cache**: 블록 단위 I/O 캐시(전통적)
* **Page cache**: 가상메모리 페이지 단위로 파일 데이터를 캐시(현대 OS에서 핵심)
* 현대 OS는 보통 둘을 통합하는 방향(일명 **Unified Buffer Cache**)을 채택해

  * 중복 캐싱을 줄이고
  * 파일 I/O와 메모리 매핑 I/O의 일관성을 개선

### 3.2 Write 정책: Write-through vs Write-back

* **Write-through**: 쓰기 즉시 디스크 반영(단순, 느릴 수 있음)
* **Write-back(Delayed write)**: 메모리에 먼저 반영하고 나중에 디스크로 flush

  * 성능은 좋지만 크래시 시 위험 → 저널링/로그/동기화 API로 보강

### 3.3 Memory-Mapped Files와의 연결

* 파일을 주소 공간에 매핑하면

  * 읽기: 페이지 폴트로 필요 블록 로드
  * 쓰기: 더티 페이지가 flush될 때 디스크 반영
* 따라서 VM과 FS는 **페이지 캐시**를 중심으로 강하게 결합된다.

---

## 4. File-System Consistency (일관성)와 Recovery (복구)

### 4.1 크래시가 만드는 문제

* 파일시스템 메타데이터는 여러 구조가 함께 맞물린다.

  * 디렉터리 엔트리, inode/FCB, 자유공간 비트맵 등
* 업데이트 도중 전원이 나가면 “일부만 반영” → 불일치 발생

### 4.2 Journaling (저널링)

* 변경을 실제 구조에 반영하기 전에 **저널(로그)** 에 기록하고 커밋한 뒤 적용
* 장점

  * 재부팅 후 전체 스캔(fsck) 대신 **저널 재생(replay)** 으로 빠른 복구
* 대표 방식(개념)

  * **Metadata journaling**: 메타데이터만 로그
  * **Full data journaling**: 데이터까지 로그(안전하지만 비용↑)
  * **Ordered**: 데이터가 먼저 디스크에 기록된 뒤 메타데이터 커밋(절충)

### 4.3 Log-Structured File System (LFS, 로그 구조 파일시스템)

* 업데이트를 “제자리 덮어쓰기” 대신 **연속 로그처럼 끝에 append**
* 장점

  * 순차 쓰기 중심이라 쓰기 성능 좋음(특히 특정 매체에서 유리)
* 단점

  * 시간이 지나면 단편화/죽은 공간이 쌓임 → **Cleaning(정리/가비지 컬렉션)** 필요

---

## 5. Advanced On-Disk Structures (고급 온디스크 구조)

### 5.1 Extents (익스텐트)

* 파일을 “블록 하나씩”이 아니라

  * **연속 블록 구간(start, length)** 들의 리스트로 표현
* 효과

  * 큰 파일에서 메타데이터 오버헤드 감소
  * 순차 I/O 성능 개선, 단편화 관리에 유리

### 5.2 Copy-on-Write (COW) 파일시스템 개념

* 기존 블록을 덮어쓰지 않고, 새 블록에 쓰고 포인터를 전환
* 장점
...
* 단점
  ...
  * 메타데이터 갱신이 많아질 수 있고, 단편화/GC 이슈가 생길 수 있음

### 5.3 Snapshots (스냅샷) 개념

* 특정 시점의 파일시스템 상태를 “논리적으로 고정”
* COW와 잘 맞고, 백업/복구/테스트에 유용

---

## 6. Special-purpose and Network File Systems (특수/네트워크 파일시스템)

### 6.1 Special-purpose FS

* `/proc` 같은 **가상 파일시스템**: 디스크에 저장된 파일이 아니라 커널 상태를 파일처럼 노출
* 장점: 도구/프로그램이 파일 I/O 모델로 시스템 정보를 다룰 수 있음

### 6.2 Network File Systems (개념)

* 원격 파일을 로컬처럼 보이게 하지만

  * 지연/캐시/동시성 때문에 **일관성 모델**이 중요
* VFS는 로컬/원격 FS를 같은 인터페이스로 연결하는 데 큰 역할

---

## 7. Summary (요약)

* 15장은 파일시스템이 실제로 동작하는 내부 메커니즘: **볼륨/마운트, VFS, 캐시, 복구, 고급 구조**를 묶어 설명한다.
* **VFS** 는 다양한 파일시스템을 공통 API로 추상화하고, name cache/inode(vnode) cache로 성능을 끌어올린다.
* 현대 OS는 **페이지 캐시 중심**으로 파일 I/O와 메모리 매핑 I/O를 통합하며, 쓰기 정책(write-back)은 저널링 등으로 안전성을 보강한다.
* 크래시 일관성은 **저널링/로그 구조** 같은 복구 메커니즘이 핵심이며,
  성능·기능 확장은 **extent, COW, snapshot** 같은 고급 구조로 지원한다.
